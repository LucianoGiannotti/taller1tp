...
bool loadMedia()
{
	...
	//Load player texture
	if( !gPlayerTexture.loadFromFile( "C:\\gametest\\bin\\Debug\\Player1.png" ) )
	{
	...
	if( !gBGTexture.loadFromFile( "C:\\gametest\\bin\\Debug\\bg.png" ) )
	{
		printf( "Failed to load background texture!\n" );
		success = false;
	}

    //Load second background texture
	if( !gBGTexture2.loadFromFile( "C:\\gametest\\bin\\Debug\\bg2.png" ) )
	{
		printf( "Failed to load background texture!\n" );
		success = false;
	}
	...
	else
	{
		...
		else
		{
			...
			while( !quit )
			{
				...
				gBGTexture2.render( 1000, 0, &camera);
